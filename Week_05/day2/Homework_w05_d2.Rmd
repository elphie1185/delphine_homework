...
```{r}
df <- subset(qb_monthly_sales, Cashflow == "Kwikbit Unit 1 Sales")
df
```


```{r}
sales <- c(qb_monthly_sales[12, 3]*1.1, 
           qb_monthly_sales[12, 3]*1.1*1.1, 
           qb_monthly_sales[12, 3]*1.1*1.1*1.1)
date <- as.Date(c("2019-08-01", "2020-08-01", "2021-08-01"))

future_sale <- data.frame(date = date, 
                          sales = sales)
```
...
ggplot(subset(qb_monthly_sales, Cashflow == "Kwikbit Unit 1 Sales")) +
  ...
  labs(
    x = "Date", 
    y = "Sales Amount", 
    title = "Sales forecast until 2021"
  ) +
  theme(
    panel.background = element_rect(fill = "white"), 
    panel.grid = element_line(colour = "grey25", size = 0.1),
